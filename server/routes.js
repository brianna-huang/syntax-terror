...
const search_songs = async function(req, res) {
  ...
  connection.query(`
    ...
    WHERE title LIKE '%${title}%' 
      ...
      AND plays <= ${playsHigh} AND plays >= ${playsLow}
      AND danceability <= ${danceabilityHigh} AND danceability >= ${danceabilityLow}
      AND energy <= ${energyHigh} AND energy >= ${energyLow}
      AND valence <= ${valenceHigh} AND valence >= ${valenceLow}
    ORDER BY title
  `, (err, data) => {
    if (err || data.length === 0) {
      console.log(err);
